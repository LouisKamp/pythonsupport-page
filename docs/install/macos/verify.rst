...
1. Search for :menuselection:`Terminal` using your Spotlight search
   (:kbd:`Command+Space`) and press :kbd:`Enter`. Verify that ``(base)``
   is shown to the left of your username. Something like the image
   below:
...
2. Type ``idle3`` in the Terminal, then press :kbd:`Enter`. This should
   open a new window in which you can run Python code.
...
- Ensure that Miniconda is installed, follow :ref:`these instructions
  <install-python-macos-conda>`.
- Ensure the packages are installed (if they are already installed, this
  will not do anything).

  Paste the following line of code to the Terminal and press
  :kbd:`Enter`:
